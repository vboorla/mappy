L.Mytrack = L.Layer.extend({
    ...
    onAdd: function(map) {
        ...
        featureGroup.addLayer(this.myLayer);
    },
    _onclick: function(e) {
        if(e.latlng.lng!=this.lng0) {
            this.lng0=e.latlng.lng;
            this.gpx.coordinates.push([e.latlng.lng,e.latlng.lat]); 
            this.myLayer.addData(this.gpx); 
            this._map.panTo([e.latlng.lat,e.latlng.lng])
        }
    }
});

L.mytrack = function(options) {
    return new L.Mytrack(options);
}; //L.mytrack = new L.Mytrack();
